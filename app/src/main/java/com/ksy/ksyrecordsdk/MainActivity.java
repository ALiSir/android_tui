...
public class MainActivity extends AppCompatActivity implements OrientationActivity, KsyRecordClient.NetworkChangeListener, KsyRecordClient.PushStreamStateListener, KsyRecordClient.SwitchCameraStateListener, KsyRecordClient.StartListener, OnClientErrorListener {
    ...
    private static final String TAG = "MianActivity,程序入口";
            ...
            timer.schedule(new TimerTask() {
                ...
                public void run() {
                    bitrate.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            Log.i(TAG, "run:每隔一秒调用一次显示！ ");
                            //定时器实时显示当前监控信息
                            bitrate.setText("推送URL地址：" + config.getUrl() + "\n" + KsyRecordSender.getRecordInstance().getAVBitrate() + "\nrecord angle =" + KsyRecordClientConfig.recordOrientation + "\npreview angel =" + KsyRecordClientConfig.previewOrientation);
...
//        client.setCameraSizeChangedListener(mSurfaceView);
    ...
    private void startRecord() {
        ...
        try {
            ...
            e.printStackTrace();
            Log.d(Constants.LOG_TAG, "录像异常 ： " + e.getMessage());
        }
    }
    ...
    * */
    ...
    public void onClientError(int source, int what) {
        Toast.makeText(MainActivity.this, "客户端错误：" + source, Toast.LENGTH_SHORT).show();
    }
}
